...
if (isset($_POST['btn-update'])) {
 ...
 if ($db->query($query)) {
      ...
      <span class='glyphicon glyphicon-info-sign'></span> &nbsp; Event updated Successfully!
     </div>";
  } else {
   $msg = "<div class='alert alert-danger'>
      <span class='glyphicon glyphicon-info-sign'></span> &nbsp; Error updating event!
     </div>";
  }
}
if (isset($_POST['btn-add'])) {
 ...
 $tinsert = $db->prepare('INSERT INTO events (id, tid, name, start, end, details) VALUES (NULL, :tid, :name, :start, :end, :details)');
 $tinsert->execute(array(':tid' => $tid, ':name' => $name, ':start' => $start, ':end' => $end, ':details' => $details));
 $affected_rows = $tinsert->rowCount();
 if ($affected_rows) {
	   $msg = "<div class='alert alert-success'>
      <span class='glyphicon glyphicon-info-sign'></span> &nbsp; Event created Successfully!
     </div>";
  } else {
   $msg = "<div class='alert alert-danger'>
...
<div class="form-group">
    <textarea class="form-control" placeholder="<Event Detail Text>" name="details" maxlength="600" rows="3" cols="132"></textarea>
</div>
</form>
...
<div class="form-group">
	...
	$aRow['start'] = str_replace(' ', 'T', $aRow['start']);
	echo $aRow['start'].'" name="start">
</div>
<div class="form-group">
    <input type="datetime-local" class="form-control" value="';
	$aRow['end'] = str_replace(' ', 'T', $aRow['end']);
	echo $aRow['end'].'" name="end">
</div>
<div class="form-group">
  <select class="form-control" name="team_or_site">
